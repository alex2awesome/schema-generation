...
EXPERIMENT_NAME="news-discourse"
EXPERIMENT_DIR="experiments/$EXPERIMENT_NAME"
INPUT_FILE="data/reparsed-newsworthiness-df.jsonl"
USE_OPENAI=true
SUMMARIZER_MODEL_NAME="gpt-4o"
LABELING_MODEL_NAME="gpt-4o-mini"
BATCH_SIZE=500
...
STEP1_OUTPUT_FILE="$EXPERIMENT_DIR/$EXPERIMENT_NAME-initial-labeling-labeling__experiment-${EXPERIMENT_NAME}__model_${LABELING_MODEL_NAME//\//-}__0_${DEFAULT_END_IDX}.json"
if [ ! -f "$STEP1_OUTPUT_FILE" ]; then
    STEP1_OUTPUT_FILE=$(ls $EXPERIMENT_DIR/${EXPERIMENT_NAME}-initial-labeling-labeling__experiment-${EXPERIMENT_NAME}__model_${LABELING_MODEL_NAME//\//-}__*_*.json | sort -V | tail -n 1)
fi
...
python src/step_2__create_supervised_similarity_data.py \
    ...
    --output_file $EXPERIMENT_DIR/${LABELING_MODEL_NAME//\//-}-similarity-data.jsonl \
    --model_name $LABELING_MODEL_NAME \
    --use_openai \
    --batch_size $BATCH_SIZE \
    --text_col_name label \
...
python src/step_3__train_sentence_similarity_model.py \
    ...
    --data_file $EXPERIMENT_DIR/triplets_${LABELING_MODEL_NAME//\//-}-similarity-data.jsonl \
...
python src/step_4__merge_labels.py \
    ...
    --datapoint_embeddings_path $EXPERIMENT_DIR/models/datapoint_embeddings.npz
...
python src/step_6__agglomerative_clustering.py \
    ...
    --experiment $EXPERIMENT_NAME \
    --raw_data_examples_df_path $EXPERIMENT_DIR/data/sentence-level-dataset.csv \
    ...
    --checking_model $LABELING_MODEL_NAME \
    --use_discretization \
    --discretization_goal "determine the common discourse role of the text in an overall news article" \
    --initial_datapoint_level_label_embeddings "${EXPERIMENT_DIR}/models/datapoint_embeddings.npz" \
    --batch_size 30 \
